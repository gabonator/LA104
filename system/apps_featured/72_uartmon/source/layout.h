namespace Layout
{
    ...
    public:
        ...
        {
            char text[2] = {c, 0};
            mX += BIOS::LCD::Print(mX, mRect.top, mF, mB, text);
            return *this;
        }*/
    ...
    class Radio : public Sublayout
    {
        ...
        const char* mName;
    public:
        Radio(bool b, const char* name) : mB(b), mName(name)
        {
        }
        ...
        virtual void Visit(Render* r) const
        {
            if (!r->IsSelected())
            {
                *r << Color(RGB565(b0b0b0), RGBTRANS) << ' ' << (mB ? 'X' : 'x') << Select(false);
                *r << Color(RGB565(b0b0b0), RGBTRANS) << ' ' << Color(RGB565(b0b0b0), RGBTRANS) << mName;
            } else
            {
                *r << Color(RGBTRANS, RGB565(ffffff)) << '<' << (mB ? 'X' : 'x') << Select(false);
                *r << Color(RGBTRANS, RGB565(ffffff)) << '>' << Color(RGB565(ffffff), RGBTRANS) << mName << Color(RGB565(b0b0b0), RGBTRANS);
            }
        }
    };
    ...
    public:
        Window(const char* label, bool redraw) : mLabel(label), mRedraw(redraw)
        {
        }

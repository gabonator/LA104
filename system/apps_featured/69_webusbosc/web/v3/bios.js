...
var OSC = {
  Enums: {"CH1": 0, "CH2": 1, "CH3": 2, "CH4": 3,
    ...
    "GreaterDTHigh":7, "None":8, "Streaming":9,
    // timebase
...
//  Restart: () => Promise.resolve() /*BIOS.rpcCall('OSC::Restart();')*/,
...
*/
    ...
    .then((json) => 
    {
      var resp = BIOS.safeeval(json);
      if (json.hex)
        throw "Wrong version of application, newer uses binary transfers!";

      var requestBytes = resp.bulk;

      return new Promise((resolve, reject) =>
      {
        COMM._onReceive = data =>
        {
          if (data.byteLength != requestBytes)
            throw "Incorrect response length!";
          var response = new Uint8Array(data.buffer);

          COMM._onReceive = () => { delete COMM._onReceive; resolve(response); }; // should send "{ret:0}" after binary transfer
        }
      });
/*
      return requestData()
...
var BIOS =
{
  ...
  SetParam: (attr, value) => BIOS.rpcCall('OSC::SetParam('+attr+','+value+');'),
};

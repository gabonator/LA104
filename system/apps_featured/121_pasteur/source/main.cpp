    ...
    while (*text)
    {
        ...
        for (ui8 _y=0; _y<14; _y++)
        {
            ...
            for (ui8 _x=0; _x<8; _x++, col <<= 1)
                if ( col & 128 )
                    BIOS::LCD::Bar(x + _x*2, y + _y*2, x + _x*2+2, y + _y*2+2, c);
        }
        x += 8*2;
    }
    return x - oldx;
...
public:
    ...
    bool Demo(CArray<int>& meas)
    {
        ...
        meas[0] = (int)((sin(now*0.00002)*30+50)*16);
        meas[1] = meas[0] - 10;
        meas[2] = meas[0] + 10;
        return true;
    }
...
public:
    ...
    void OnPaint()
    {
        ...
        BIOS::LCD::Print(8, 0, RGB565(ffffff), RGBTRANS, "Pasteurisation computer");

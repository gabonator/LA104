...
public:
    ...
    virtual void OnPaint() override
    {
        ...
        for (int i=0; (item = GetItem(i)).strName; i++)
        {
            ...
            {
                case TItem::Static:
                    BIOS::LCD::Bar(x, y, x+4+8, y+14, RGB565(b0b0b0));
                    x += 4+8;
                ...
                case TItem::Selected:
                    if (HasFocus())
                    {
                        x += BIOS::LCD::Draw( x, y, RGB565(ffffff), RGBTRANS, CShapes_tab_left);
...
public:
    virtual void OnPaint() override
    {
        ...
        int x = m_rcClient.left-4, y = m_rcClient.top+2;
        TItem item;
        for (int i=0; (item = GetItem(i)).strName; i++)
        {
            TItem::EState state = item.eState;
            ...
            switch (state)
            {
                case TItem::Static:
                    ...
                    break;
                case TItem::Default:
                    x += BIOS::LCD::Draw( x, y, RGB565(808080), RGBTRANS, CShapes_tab_left);
                    x += BIOS::LCD::Print(x, y, RGB565(404040), RGB565(808080), item.strName);
                    x += BIOS::LCD::Draw( x, y, RGB565(808080), RGBTRANS, CShapes_tab_right);
                    break;
                case TItem::Selected:
                    if (HasFocus())
                    {
                        x += BIOS::LCD::Draw( x, y, RGB565(ffffff), RGBTRANS, CShapes_tab_left);
                        x += BIOS::LCD::Print(x, y, RGB565(404040), RGB565(ffffff), item.strName);
                        x += BIOS::LCD::Draw( x, y, RGB565(ffffff), RGBTRANS, CShapes_tab_right);
                    } else
                    {
                        x += BIOS::LCD::Draw( x, y, RGB565(404040), RGBTRANS, CShapes_tab_left);

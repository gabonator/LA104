...
private:
  ...
  {                                   
    ...
    virtual void GetDescription(CAttributes& attributes, char* desc) override
    {
        ...
        {          
            sprintf(desc, "<WGR800:> avg <%d.%d m/s> gusts <%d.%d>", (int)attributes["wind10"] / 10, (int)attributes["wind10"] % 10,
              (int)attributes["gusts10"] / 10, (int)attributes["gusts10"] % 10);
        }
        else if (model == String_PCR800)
        {
            sprintf(desc, "<PCR800:> rain <%d.%d\"/h> total <%d.%d\">", (int)attributes["rainRate100i"] / 100, (int)(attributes["rainRate100i"] % 100)/10, 

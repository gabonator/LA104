...
{
	target *t = (void*)alloc_target(sizeof(*t));
                                ...
                                target_addr dest, const void *src, size_t len)
{
	...
	if (f->buf == NULL) {
		...
		f->buf = malloc_sector(f->buf_size);
                          ...
                          enum target_breakwatch type, target_addr addr, size_t len)
{
	...
	if (ret == 0) {
		...
		struct breakwatch *bwm = malloc_breakwatch(sizeof bw);

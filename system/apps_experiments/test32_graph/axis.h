...
protected:
    ...
    virtual void Redraw(int value, int origValue)
    {
        ...
        for (int _i=0; _i<=value; _i += step)
        {
            ...
            BIOS::LCD::Print(tx - width/2, m_rcClient.bottom-14, RGB565(ffffff), RGB565(b0b0b0), formatted);
...
public:
    ...
    void Update(const Range& range)
    {
        ...
        auto drawTick = [&](int i)
        {
            ...
            BIOS::LCD::Print(m_rcClient.right - strlen(buffer)*8-3, ty, RGB565(ffffff), RGB565(b0b0b0), buffer);
        };

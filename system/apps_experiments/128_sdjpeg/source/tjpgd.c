...
)
{
    ...
    if (jd->swap)
    {
        ...
        for (int yy=0; yy<ry; yy++)
            for (int xx=yy+1; xx<rx; xx++)
            {
                tt = d[xx*ry+yy];
                d[xx*ry+yy] = d[yy*rx+xx];
                d[yy*rx+xx] = tt;
            }
        for (int yy=0; yy<ry/2; yy++)
            for (int xx=0; xx<rx; xx++)
            {
                tt = d[xx*ry+yy];
                d[xx*ry+yy] = d[xx*ry+ry-1-yy];
                d[xx*ry+ry-1-yy] = tt;
            }
     /*
        memset(d, 0, 16*16*2);
        ...
        {
            //uint16_t* pd = d + x*ry;//(rx-1-x)*ry; /// + (rx-1-x)*ry;
            for (int lx=0; lx<16; lx++)
            {
                w =  (*s++ & 0xF8) << 8;    // RRRRR-----------
                w |= (*s++ & 0xFC) << 3;    // -----GGGGGG-----
                w |= *s++ >> 3;             // -----------BBBBB
                d[lx+ly*16] = w; //[x*rx+y] = w; // Swap bytes
            }
        }*/

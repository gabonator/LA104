...
uc16 FONT_8x11[]  =  {0x008,0x004,0x008,0x020,0x080,0x100,0x080,0x020, // " 
                      0x010,0x004,0x010,0x040,0x100,0x400,0x100,0x040, // # 
                      0x200,0x100,0x080,0x040,0x020,0x010,0x008,0x3FC, // $ 
                      ...
                      0x004,0x004,0x004,0x3FC,0x200,0x200,0x200,0x3FC, // , 
                      ...
                      0x0F8,0x088,0x38E,0x222,0x2FA,0x2FA,0x2FA,0x2FA, // { 
...
uc16 Palette[] = {CYAN, YEL, PRPL, GRN, CYAN_, YEL_, PRPL_, GRN_, // 
                  BLK,  RED, WHT,  GRN, BLUE,  GRAY, ORNG,  DARK};
                 
uc16 HueTab[4] = {GRAY, CYAN, YEL, WHT}; // Pop Window ɫ

u8   PopHide = 1;              // Pop Window 
u32  PopBuf[1500];             // Pop Window 
u16  Nx0, Ny0;                 // Pop Window 
u16  Bx1, Bx2, By1, By2;       // Pop Window 
u8   YnHide[4] = {0, 0, 0, 0}; // Track1~4
u8   Track[(X_SIZE+1)*4];      // 
...
{ 
  ...
  PopSTR(Pop[7].Src, 0, (u8*)Pop[7].STR+11);     // 
  Nx0 = Pop[8].X0, Ny0 = Pop[8].Y0;
  if(Line >= SVOL) r = SaveParam();              // 
  ...
  u8 n = (r == OK) ? 11 : 22;
  PopSTR(Pop[8].Src, 0, (u8*)Pop[8].STR+n); // 
  DisplayWaveForm(); 
  Delay_mS(1000);
  PopUpdt = 1;
}
...
void PopSTR(u8 Hue, u8 Inv, u8 *s)
{
  ...
  while(*s != 0){
    Nx0--;
    for(u32 i = 0; i < 12; i++) PopPixle(ForeGrnd); // 
    Nx0++, Ny0 -= 12;
    for(u32 i = 0; i < 8; i++){
...
void PopSTR(u8 Hue, u8 Inv, const char *s)
{
  PopSTR(Hue, Inv, (u8*)s);
}
...
void AddStr(u8 ColorNo, u8 Mode, const char *Str)
{
  AddStr(ColorNo, Mode, (u8*)Str);
}
/*******************************************************************************
 PrintClk: 
*******************************************************************************/
...
void DrawTags(void)
{
  ...
  LCD_WrBlock(0, MIN_Y-1, 10, MIN_Y+Y_SIZE+1); // 
  for(u16 x = 0; x <= 10; x++){
    u16* p = (x & 1) ? Buf1 : Buf0; 
    u32* p32 = (u32*)p;
    ...
    for(u32 i = 0; i < 4; i++){
      ...
      if(!YnHide[i] && (Item != YNP || Menu[YNP].Src != i || Flashing)){
        if(Posi >= 3 || Posi <= Y_SIZE-3){
          ...
          } else  for(int y = x-6; y < (15-x); y++) p[Posi+y] |= Color;
        }
      }
    }
    Hw.pLCD_DmaWait();
...
void UpdateTag(void)
{
  ...
  s16 T0P = (Menu[T_0].Val-Menu[XNP].Val)*30; // T0 
...
{ 
  ...
  u16 T0P = (Menu[T_0].Val-Menu[XNP].Val)*30;                // T0 
  u16 T1P = Menu[T_1].Val, T2P = Menu[T_2].Val;              // T1,T2 
  u16 V1P = Menu[V_1].Val, V2P = Menu[V_2].Val;              // V1,V2 
  u16 VtS = Menu[V_T].Src;                                   // Vt 
  u16 VtP = Vt[VtS];                                         // Vt 
  u16 VtC = Palette[VtS];                                    // Vt 
  u16 GrC = Palette[GRIDC];                                  // 
  u16 BgC = Palette[BLANK];                                  // 
  u16 CsC = Palette[CURSR];                                  // V/T1~2 

  u16 T0F = Menu[T_0].Flg & INVR;                            // T0 
  u16 T1F = Menu[T_1].Flg & INVR;                            // T1 
  u16 T2F = Menu[T_2].Flg & INVR;                            // T1 
  u16 V1F = Menu[V_1].Flg & INVR;                            // V1 
  u16 V2F = Menu[V_2].Flg & INVR;                            // V2 
  u16 VtF = Menu[V_T].Flg & INVR;                            // Vt 
  
  FpsCnt++;
  LCD_WrBlock(MIN_X, MIN_Y, MIN_X+X_SIZE, MIN_Y+Y_SIZE);          // 
  for(int Col = X_BASE; Col <= X_SIZE; Col++){
    u16* p = (Col & 1) ? Buf1 : Buf0;                             // 
    u32* p32 = (u32*)p;
    u16 Fill = (Col == X_BASE || Col == X_SIZE) ? GrC : BgC;
    for(u32 i = X_BASE; i <= Y_SIZE/2; i++) p32[i] = Fill;        // 
    p[Y_SIZE] = GrC, p[Y_BASE] = GrC;                             // 
    if(Col%30 == 0) for(u32 y = 5;  y < 200; y +=  5) p[y] = GrC; // 
    if(Col% 6 == 0) for(u32 x = 25; x < 200; x += 25) p[x] = GrC; // 
    ...
    if(Col != X_BASE && Col != X_SIZE){
      for(u32 n = 0; n < 4; n++){
        ...
        if(Track[m] != 0xFF){                                     // 
          ...
          u16 TrkC = Palette[n];
          if((Max[n]-Min[n]) > 5) TrkC &= 0xBDF7;                 // 
          if(Col != X_BASE && Col != X_SIZE && !YnHide[n]){
            for(u32 y = Min[n]; y <= Max[n]; y++){
              if(Min[n] > Y_BASE && Max[n] < Y_SIZE) p[y] |= TrkC;// 
            }
          }
        }
      }
      ...
      p[V1P] |= CsC, p[V1P-1] |= CsC, p[V1P+1] |= CsC;       // 
      p[V2P] |= CsC, p[V2P-1] |= CsC, p[V2P+1] |= CsC;       // 
      p[VtP] |= VtC, p[VtP-1] |= VtC, p[VtP+1] |= VtC;       // 
    }
    if(Col == X_BASE+1 || Col == X_SIZE-1){
      p[V1P] |= CsC, p[V2P] |= CsC, p[VtP] |= VtC;           // 
    }
    if(Col == T1P && T1F)
      for(u32 y = 1; y <= Y_SIZE; y += 3) p[y] |= CsC;       // 
    if(Col == T2P && T2F) 
      for(u32 y = 1; y <= Y_SIZE; y += 3) p[y] |= CsC;       // 
    ...
    if(Col == T0P){
      if(T0F) for(u32 y = 1; y <= Y_SIZE; y += 3) p[y] |= VtC; // 
      p[Y_SIZE] = VtC, p[Y_SIZE-1] = VtC;
      p[Y_BASE] = VtC, p[Y_BASE+1] = VtC;                      // 
    }

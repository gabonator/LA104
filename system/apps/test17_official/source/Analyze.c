...
uc16 BaudRate[10] = { 48, 96, 144, 192, 384, 560, 576, 1152 }; // 
...
//  u8 Data_Format = gInputSubParam[IN_SPI][SPI_DATA_FORMAT];
    ...
    SCK_Last = CLK_Polarity;                               // ʱ

    for(i = TRI_START_NUM; i < RECORD_DEPTH; i++)
    {
        Status = TransStatusData(i);                       // 4bit
        SCK_Now = (Status >> 2) & 0x01;                    // Bit2 SCK
        if(((Status >> 3) & 0x01) == CS_Polarity)          // Bit3 CSλ
        {
//          
            if((StartFlag == 0) && (SCK_Last == CLK_Polarity) && (SCK_Now != CLK_Polarity))
            {
                StartFlag = 1;
                gSamplBuf[4 * i] |= 0xC0;                  // Э
...
void IIC_AnalyzeFrame(void)
{
    ...
    for(i = TRI_START_NUM; i < RECORD_DEPTH; i++)
    {
        SDA_Now = TransStatusData(i) & 0x01;               // ͨ
        SCL_Now = (TransStatusData(i) >> 1) & 0x01;        // ͨ
        if((k == 0) && (SCL_Now == 1) && (SCL_Last == 1) && (SDA_Last == 1) && (SDA_Now == 0))   // SCLΪ
        {
            All_Status = 1;
            StartFlag = 1;
        }
        if((Ack == 0) && (SCL_Now == 1) && (SCL_Last == 1) && (SDA_Last == 0) && (SDA_Now == 1)) // SCLΪ
        {
            for(n = 1; n <= k; n++)
            {
        ...
        if(All_Status == 1)
        {
            ...
            if(k <= 9)
            {
                if(SCK_Status) gSamplBuf[4 * i] |= 0x40;   // 0x40Ϊ
                if(k == 9)
                {
                    if(StartFlag)
                    {
                        ...
                        AnalyzeBuf[j * 3 + 1] |= (AnalyzeBuf[j * 3 + 0] & 0x01) << 1; // 
...
//  
    ...
    {
//      һ
        if((StopFlag == 0) && ((gLCD_Backup[LCD_Point] & 0x10) == 0) && ((gLCD_Backup[LCD_Point + 1] & 0x10) == 0x10))      // 
        {
            StartCnt = LCD_Point + 3;
        }
        else if((StopFlag == 0) && ((gLCD_Backup[LCD_Point] & 0x10) == 0x10) && ((gLCD_Backup[LCD_Point + 1] & 0x10) == 0)) // 
        {
            StopCnt = LCD_Point;
        ...
        if(StopFlag == 1)
        {
            if(AnalyzeBuf[DateCnt * 3 + 1] & 0x01)                                 // 
            {
                if(gInputSubParam[IN_I2C][0]) AnalyzeBuf[DateCnt * 3 + 0] &= 0xFE; // R/W 
            }
            switch(Data_Format)                                                    // 
            {
            case ASCII:
            ...
            if(FrameWidth > (DecodeWidth / 2))
            {
                ...
                {
                    RW_Posi = 7 * 2;
                    Disp_Str7x9(40 + StartCnt + StartPosi, 102, BLK, WHT, PRN, THIN, (const char*)((AnalyzeBuf[DateCnt * 3 + 1] & 0x02) ? "R:" : "W:"));
                }
                else
                {
                    RW_Posi = 0;
                }
                Disp_Str7x9(40 + StartCnt + RW_Posi + StartPosi, 102, BLK, WHT, PRN, THIN, (const char*)Str);
                Disp_Str7x9(40 + StartCnt + DecodeWidth + RW_Posi + StartPosi, 102, BLK, WHT, PRN, THIN, (const char*)((AnalyzeBuf[DateCnt * 3 + 1] & 0x04) ? " ACK" : " NAK"));
            }
            StopFlag = 0;
            DateCnt++;
        }
...
//  u8  StopFlag = 0;
    ...
    for(i = TRI_START_NUM; i < RECORD_DEPTH; i++)
    {
        ...
        if((SCL_Now == 1) && (SCL_Last == 1) && (SDA_Last == 1) && (SDA_Now == 0)) // SCLΪ
        {
            ALL_Start = 1;
        }
        if((SCL_Now == 1) && (SCL_Last == 1) && (SDA_Last == 0) && (SDA_Now == 1)) // SCLΪ
        {
            ALL_Start = 0;
        }
        if(ALL_Start == 1)
...
//  BIT_UnitTime = (100000000 / (Baud*100));                     // 
    BIT_UnitTime = (1000000 / Baud) - (1000000 / Baud / 20);     // 

    for(i = TRI_START_NUM; i < RECORD_DEPTH; i++)
    {
        NowStatus = TransStatusData(i) & 0x01;                   // ͨ
        TransTime = TransformTime(i + 1);
...
//          
            ...
            if((StartFlag == 0) && (LastStatus == 1) && (NowStatus == 0))   // 
            {
                StartFlag = 1;                                              // 
                gSamplBuf[4 * i] |= 0x40;                                   // Э
...
void Show_UART_AnalyzeData(void)
{
    ...
    Baud = BaudRate[gInputSubParam[IN_UART][UART_BAUD_RATE]];
//  BIT_UnitTime = (100000000 / (Baud*100));                                         // 
    BIT_UnitTime = (1000000 / Baud);                                                 // 
    BIT_UnitWidth = BIT_UnitTime * TIME_TO_POINT / gTB_Scale[gItemParam[TIMEBASE]];  // 
    FrameWidth = BIT_UnitTime * 8 * TIME_TO_POINT / gTB_Scale[gItemParam[TIMEBASE]]; // 
...
//  
    ...
    {
//      һ
        if((gLCD_Backup[LCD_Point] & 0x10) && ((gLCD_Backup[LCD_Point + 1] & 0x10) == 0)) // 
        {
            if(LCD_Point > BIT_UnitWidth * DataBits)
            {
                ...
                {
                    Disp_Str7x9(40 + (LCD_Point - StartPosi - DecodeWidth), 102, BLK, WHT, PRN, THIN, (const char*)Str);
                }
            }
        }
        LCD_Point++;
...
//      һ
        ...
        if(StopFlag == 1)
        {
            ...
            if(FrameWidth > (DecodeWidth / 2))
            {
                Disp_Str7x9(40 + StartCnt, 102, BLK, WHT, PRN, THIN, (const char*)Str);
                Disp_Str7x9(40 + StartCnt,  77, BLK, WHT, PRN, THIN, (const char*)Str1);
            }
            StopFlag = 0;
            DateCnt++;
        }
...
u16 SPI_AnalyzeDataStartCnt(u64 StartTime)
{
    ...
    for(i = TRI_START_NUM; i < RECORD_DEPTH; i++)
    {
        ...
        if((StopFlag == 0) && (CS_Status != CS_Polarity))  // cs
        {
            StartFlag = 0;
            StopFlag = 1;
            SCK_Cnt = 0;

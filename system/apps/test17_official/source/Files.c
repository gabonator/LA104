...
//          DiskBuf[i] = (u8)(ColorH>>8);
            ...
            if(i >= length)
            {
                ...
                if(l == 0) PrintClk(SAVE_ICO_X0, SAVE_ICO_Y0, (k++ >> 1) & 3); // 
                l++;
                if(l >= 2) l = 0;
            }
...
u8 SaveParameter(void)
{
    ...
    if(j == FLASH_COMPLETE)
    {
        ...
        *ptr++ = 0x0100;                                           // 
        *ptr++ = Menu.index;                                       // 
        for(i = 0; i < MENU_NUM; i++) *ptr++ = gItemIndexNum[i];             // 
        for(i = 0; i < ITEM_NUM; i++) *ptr++ = gInSubIndexNum[IN_TYPE][i];   // 
        for(i = 0; i < ITEM_NUM; i++) *ptr++ = gOutSubIndexNum[OUT_TYPE][i]; // 
        for(i = 0; i < PARAM_NUM; i++) *ptr++ = gItemParam[i];               // ȫ
        for(i = 0; i < ITEM_NUM; i++)
        {
            for(k = 0; k < SUB_NUM; k++) *ptr++ = gInputSubParam[i][k];      // input
            for(k = 0; k < SUB_NUM; k++) *ptr++ = gOutSubParam[i][k];        // input
        }
        *ptr++ = 0xaa55;
        ptr = (u16 *)&DiskBuf;
        for(i = 0; i < 512; i += 2)                                         
...
u8 ReadParameter(void)
{
    ...
    Menu.index = *ptr++;
    for(i = 0; i < MENU_NUM; i++) gItemIndexNum[i] = *ptr++;             // 
    for(i = 0; i < ITEM_NUM; i++) gInSubIndexNum[IN_TYPE][i] = *ptr++;   // 
    for(i = 0; i < ITEM_NUM; i++) gOutSubIndexNum[OUT_TYPE][i] = *ptr++; // 
    for(i = 0; i < PARAM_NUM; i++) gItemParam[i] = *ptr++;               // ȫ
    for(i = 0; i < ITEM_NUM; i++)
    {
        for(j = 0; j < SUB_NUM; j++) gInputSubParam[i][j] = *ptr++;      // input
        for(j = 0; j < SUB_NUM; j++) gOutSubParam[i][j] = *ptr++;        // output
    }
...
u8 RestoreParameter(void)
{
    ...
    Menu.index = 0;
    Item.index = 0;
    for(i = 0; i < MENU_NUM; i++) gItemIndexNum[i] = gItemIndexNumBackup[i]; // 
    for(i = 0; i < ITEM_NUM; i++) gInSubIndexNum[IN_TYPE][i] = gInSubIndexNumBackup[IN_TYPE][i];     // 
    for(i = 0; i < ITEM_NUM; i++) gOutSubIndexNum[OUT_TYPE][i] = gOutSubIndexNumBackup[OUT_TYPE][i]; // 
    for(i = 0; i < PARAM_NUM; i++) gItemParam[i] = gItemParamBackup[i];      // ȫ
    for(i = 0; i < ITEM_NUM; i++)
    {

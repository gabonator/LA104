...
WS2812B::WS2812B(uint16_t number_of_leds) :
  brightness(0), pixels(nullptr), begun(false)
{
...
if (!begun)
{
  SPI.setClockDivider(CSPI::SPI_CLOCK_DIV32);// need bit rate of 400nS but closest we can do @ 72Mhz is 444ns (which is within spec)
  SPI.begin();
  begun = true;
}
}

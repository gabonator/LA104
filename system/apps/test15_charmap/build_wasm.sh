...
emcc ${INCLUDES} ${SOURCE} -g4 -O3 -s TOTAL_STACK=1024 -s TOTAL_MEMORY=65536 -s ENVIRONMENT="web" -s TEXTDECODER=2 -s MALLOC=emmalloc -s WASM=1 -s EXPORTED_FUNCTIONS="${EXPORTED}" -o charmap.js -DEMULATED -DEMSCRIPTEN -DLA104 --std=c++11 -s ERROR_ON_UNDEFINED_SYMBOLS=0 || exit 1
node ../../../os_platform/wasm/htmllite/package.js charmap

...
cd app
af.exe -B -X "exit [catch {generate_pll {inpll} -pll_type {PLLX} -input_freq {2} -output_freqs { {25}} -fb_mode {NO_REFERENCE}}]"
af.exe -B --setup --mode QUARTUS --design app --device AG1KLPQ48 --ip %cd%\inpll.ip -X "set DEVICE_FAMILY 1"

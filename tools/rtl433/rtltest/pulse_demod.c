...
int pulse_demod_manchester_zerobit(const pulse_data_t *pulses, r_device *device)
{
	...
	/*
printf("demodulujem '%s' len=%d range=%d..%d / %d..%d\n", device->name, pulses->num_pulses, device->s_short_width - device->s_tolerance, device->s_short_width + device->s_tolerance, device->s_short_width * 2 - device->s_tolerance, device->s_short_width * 2 + device->s_tolerance);
	*/
